...
DATA_PATH = os.path.join("./Data-Sets/")
RECIPES_PATH = os.path.join(DATA_PATH, "embeddings.json")

# Import data sets
df = pd.read_json(RECIPES_PATH)
print("Loaded {} recipes with {} columns".format(df.shape[0], df.shape[1]))
...
def recommend(query, top_n):
    ...
    return df.embeddings.apply(
        lambda recipe: util.pytorch_cos_sim(query_embedd, recipe).detach().numpy().flatten()
...
def generate_combinations(ingredients_list):
    ...
    reccommentations = recommend(ingredients_list, 10)
    for key, _ in reccommentations.items():
        directions = '\n-> '.join(df.loc[key].directions)
        ans[0].append({'author':'No Author', 'title': df.loc[key]["name"],'content': directions, 'isFavorite':"false", 'classname':"heart"})
...
def addRecord(username, email, password):
    ...
    if results:
        ...
        bio = "Hey there! I am using RecipeBowl."
